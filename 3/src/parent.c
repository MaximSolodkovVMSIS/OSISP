...
void signal2_handler(int signo) {} //обработчки сигналов 
...
int main(int argc, char** argv, char** envp)
{
    ...
    signal(SIGUSR1, signal1_handler); //пользовательские сигналы
    ...
    while (scanf("%s", input))
    {
        ...
        case 's':
        {
            if (strlen(input) > 1)
            {
                if(child_count < atoi(input + 1)) //преобразуем строку в число
